...
public class Problem2 {
    ...
    public Problem2(String graphContent) {
        nonDirectedGraph = new HashMap<>();
        buildGraph(graphContent);
    }
    public Problem2(int[][] graphContent, String sourceNode) {
        nonDirectedGraph = new HashMap<>();
    ...
    private void buildGraph(int[][] graph, String sourceNode){
        //String[] lines = graphContent.split("\n");
        String startNodeLabel, endNodeLabel;
        Vertex startNode, endNode;
        ...
        startVertex = nonDirectedGraph.get(sourceNode);//总是以文件中第一行第二列的那个标识顶点作为源点
    }
